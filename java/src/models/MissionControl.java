...
public class MissionControl {
	...
	public boolean addRover(Program program, Rover rover) {
		...
		return this.roversWithPrograms.add(
			new RoverProgramPair(program, rover)
		);
	}
	...
	public boolean isMoveLegal(Rover rover, Position newPosition) {
		...
		boolean doesNotCollide = roversWithPrograms.stream()
			.map(RoverProgramPair::getRover)
	        .noneMatch(r -> {
	        	
	        	boolean collides = !r.equals(rover) && r.getPosition().equals(newPosition);
	...
	private void moveRoversSequentially() {
		...
		for(RoverProgramPair roverProgramPair : this.roversWithPrograms) {
			...
			while(program.hasNext()) {
				...
				Position nextPotentialPosition = rover.turnOrCalculateNextPosition(nextInstruction);
				...
				if(this.isMoveLegal(rover, nextPotentialPosition)) {
					...
					// move,
					System.err.println("Rover " + roverCounter + " made an illegal move, stopping at " + rover.getPosition() + " ...");
