...
class ApplicationLaunchTest {
  ...
  void testTestProram() {
    assertDoesNotThrow(() ->
      ApplicationLaunch.main(
        new String[] { "./testprograms/exampleprogram.txt" }
      )
    );
  ...
  void testTestProramInvalidPlacement() {
    assertDoesNotThrow(() ->
      ApplicationLaunch.main(
        new String[] { "./testprograms/testprograminvalidplacement.txt" }
      )
    );
  ...
  void testTestProgramRoverWantsToLeavePlateauSoShouldBeOnEdge() {
    assertDoesNotThrow(() ->
      ApplicationLaunch.main(
        new String[] { "./testprograms/testprogramrovergoesoutofbounds.txt" }
      )
    );
  ...
  void testTestProgramRoverCrashesIntoOther() {
    assertDoesNotThrow(() ->
      ApplicationLaunch.main(
        new String[] {
          "./testprograms/testprogramtworoverssecondonecrashes.txt",
        }
      )
    );
  ...
  void testTestIllegalProgram() {
    ApplicationLaunch.main(
      new String[] { "./testprograms/illegalinstructiononerover.txt" }
    );

    assertEquals(
      "Invalid program. Only 'L', 'R', and 'M' are allowed.",
      captureError.toString().trim()
    );
  }
  ...
  void testTestIllegalProgramForOneRoverOnly() {
    ApplicationLaunch.main(
      new String[] { "./testprograms/illegalinstructiontworovers.txt" }
    );
  ...
  void testIllegalHeading() {
    ApplicationLaunch.main(
      new String[] { "./testprograms/illegalheading.txt" }
    );
  ...
  void testInvalidPlateau() {
    ApplicationLaunch.main(
      new String[] { "./testprograms/invalidplateau.txt" }
    );
    ...
    assertTrue(
      captureError
        .toString()
        .trim()
        .contains("not running. Please fix input and try again")
    );
  }
